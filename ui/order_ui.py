...
class OrderUI:
    ...
    def get_order_menu(self, user_name) -> None:
        UI.decorate_header("(N)ew order - (R)etrieve order status - (U)pdate order status", user_name=user_name, with_footer_fill=True)

        while True:
            user_input = input("Please choose N to add new order, R to retrieve an order's status or U to update an order's status: ")
    ...
    def get_order(self) -> Order:
        while True:
            user_input = input(f"[i] Id of order: ")
    ...
    def set_priority(self) -> Priority:
        while True:
            user_input = input("[i] Order prioirty - (L)ow, (M)edium or (H)igh: ")
    ...
    def set_customer(self):
        while True:
            user_input = input(f"[i] Id of Customer: ")
    ...
    def set_vehicle(self, number_of_items, weight) -> str:
        #### TESTS ####
        # # Shipment1 Test
        # return Vehicle().get_first_available(1, 5)
        # # Shipment2 Test
        # return Vehicle().get_first_available(12, 3)
        # # Shipment3 Test
        # return Vehicle().get_first_available(60, 4500)
        # # Shipment4 Test
        # return Vehicle().get_first_available(120, 2500)
        return Vehicle().get_first_available(number_of_items, weight)
    

    def is_valid_delivery_date(self, delivery_date):
        # Step 1: Check if the input is exactly 8 characters
        if len(delivery_date) != 8:
            print("Invalid format: Input must be exactly 8 characters long.")
            return False

        # Step 2: Check if all characters are digits
        ...
        # Step 4: Use datetime to validate if it's a real date
        try:
            delivery_date = datetime(year, month, day)  # This will raise an error if the date is invalid
    ...
    def update_order_status(self, user_name) -> None:
        ...
        if(order.order_status == OrderStatus.DELIVERED or order.order_status == OrderStatus.CANCELLED):
            order.vehicle.remaining_item_capacity += len(order.items)
            order.vehicle.remaining_kg_capacity += order.total_weight
            order.vehicle.update()
